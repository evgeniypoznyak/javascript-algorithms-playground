...
const solution = (A, B, C, D) => {
  ...
  for (let i = 0; i < length; i++) {
    if (arrayToChange[i + 1]) {
      ...
      const numberToCheckNotInOrder =
        arrayToChange[i + 1] + '' + arrayToChange[i]
      ...
      for (let j = i + 2; j < length; j++) {
        ...
        const numberToCheckNotInOrderJ =
          arrayToChange[j] + '' + arrayToChange[i]
  ...
  for (let i = 0; i < properVariations.length; i++) {
    ...
    for (let j = i; j < properVariations.length; j++) {
      if (isHourIntValid(j) && isSecondIterationValueValid(numArrayCopy, j)) {
        resultArr.push({
          value: properVariations[j].value + '' + properVariations[i].value,
        })
      }

      if (isHourIntValid(i) && isSecondIterationValueValid(numArrayCopy, j)) {
        resultArr.push({
